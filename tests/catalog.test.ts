...
test('docs_search recommends the current secure 0.1.21 Codex installer flow', () => {
  const result = searchCatalog(docsIndex, 'public mcp install command', 10).find(
    entry => entry.id === 'public-mcp-install-command',
  );

  assert.ok(result);
  assert.match(result.summary, /npx --yes @spala-ai\/mcp-install@0\.1\.21 init --client codex --yes --json/);
  assert.match(result.summary, /npx --yes @spala-ai\/mcp-install@0\.1\.21 status --client codex --json/);
