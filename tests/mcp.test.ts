...
test('spala_start auto-scopes one organization and returns its projects', async () => {
  ...
  await withVerifiedClient(api, async client => {
    ...
    assert.equal((body.installerMaintenance as Record<string, unknown>).testedVersion, '0.1.28');
    assert.deepEqual(body.nextAction, {
...
test('project_connect, compatibility select, and manifest send the client and keep bootstrap capabilities out of argv', async () => {
  ...
  await withVerifiedClient(api, async client => {
    ...
    assert.deepEqual(connectPlan.argv.slice(0, 5), ['npx', '--yes', '@spala-ai/mcp-install@0.1.28', 'project', 'bind']);
    ...
    assert.deepEqual(manifestArgv.slice(0, 5), ['pnpm', 'dlx', '@spala-ai/mcp-install@0.1.28', 'project', 'bind']);
