...
```bash
npx --yes @spala-ai/mcp-install@0.1.13 init --client codex --yes --json
```
...
```txt
npx --yes @spala-ai/mcp-install@0.1.13 project bind --project-id <project-id> --project-url <exact-project-url> --url <exact-mcp-url> --name <deterministic-server-name> --client codex --install-scope workspace --bootstrap-stdin --exact-url --yes --json
```
