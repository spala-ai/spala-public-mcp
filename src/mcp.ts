...
const TOOL_DESCRIPTIONS = {
  spalaStart: [
    'AUTH REQUIRED; READ-ONLY STARTUP. Invoke after OAuth to begin Spala account or project setup.',
    'Returns a versioned startup phase with account readiness, organizations, safely scoped project discovery, and one explicit next action.',
    'Reports missing account fields, automatically scopes a sole organization, and returns all accessible choices when multiple organizations exist.',
  ].join(' '),
  spalaHelp: [
    'Returns a human-readable overview of Spala, canonical start URLs, and the boundary between public discovery and project-scoped backend MCPs.',
    'Invoke for product, setup, or integration orientation. This read-only tool requires no authentication and does not access or modify project data.',
  ].join(' '),
  onboarding: [
    'Returns machine-readable Spala onboarding information for MCP clients.',
    'Includes product scope, OAuth metadata, supported installer client identifiers, authenticated project-handoff status, workflow stages, and canonical links.',
  ].join(' '),
  toolMap: [
    'Returns a machine-readable routing map for Spala MCP clients.',
    'Identifies public and authenticated tools, supported installer clients, OAuth endpoints, project handoff rules, and project-MCP entry points.',
  ].join(' '),
  docsSearch: [
  ...
  templateList: [
    'Lists public Spala backend templates matching an optional query.',
    'Invoke to discover available backend starting points such as marketplace, dashboard API, reservation, inventory, or document management.',
  ...
  accountStatus: [
    'AUTH REQUIRED; READ-ONLY. Verifies the current public MCP OAuth identity and account readiness.',
    'Returns the authenticated identity, available organizations, and exact missing account fields without exposing OAuth or dashboard credentials.',
  ].join(' '),
  accountSetup: [
    'AUTH REQUIRED; WRITES MISSING ACCOUNT DATA TO THE SPALA CONTROL PLANE.',
    'Fills missing first and last name fields and creates the first company or workspace organization when none exists.',
    'Invoke only with real account and organization values supplied by the user.',
  ].join(' '),
  organizationCreate: [
    'AUTH REQUIRED; WRITES A NEW SPALA ORGANIZATION FOR THE SIGNED-IN ACCOUNT.',
    'Creates an additional company or workspace organization using the supplied name.',
  ].join(' '),
  projectList: [
    'AUTH REQUIRED; READ-ONLY. Lists projects accessible to the authenticated Spala user in one organization.',
    'Accepts an organization id when multiple organizations are available and automatically scopes a sole organization.',
  ].join(' '),
  projectCreate: [
