export const docsIndex = [
  ...
  },
  {
    ...
    summary: 'For Codex use npx --yes @spala-ai/mcp-install@0.1.12 init --client codex --yes --json, then run npx --yes @spala-ai/mcp-install@0.1.12 status --client codex --json and follow the exact JSON steps. After authentication, call project_connect once for workspace binding. Execute installPlan.argv as a direct process with tty:true and shell:false, wait for the running process, then send bootstrap.consumeUrl plus a newline through the process stdin tool; never put it in shell text or argv. Legacy flags remain compatibility-only; project MCP must never be installed globally.',
